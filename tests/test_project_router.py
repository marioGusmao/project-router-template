...
def load_sync_module():
    script_path = Path(__file__).resolve().parents[1] / "src" / "project_router" / "sync_client.py"
    spec = importlib.util.spec_from_file_location("project_router_client_test", script_path)
                ...
                json.dumps(
                    {
                        ...
                        "projects": {
                            "home_renovation": {
                                "display_name": "Home Renovation",
                                "language": "en",
                                "inbox_path": "/ABSOLUTE/PATH/TO/HOME_RENOVATION/Inbox/ProjectRouter",
                                "note_type": "project-idea",
                                "auto_dispatch_threshold": 0.9,
                                "keywords": ["renovation"],
                            }
...
class ProjectRouterClientTests(unittest.TestCase):
...
class BootstrapPrivateRepoTests(unittest.TestCase):
    def test_bootstrap_private_repo_writes_metadata_and_updates_managed_blocks(self) -> None:
        ...
        with temporary_repo_dir() as tmp:
            ...
            self.assertIn("private operational Project Router repo for VoiceNotes", (root / "README.md").read_text(encoding="utf-8"))
